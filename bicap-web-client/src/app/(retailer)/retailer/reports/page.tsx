...
export default function RetailerReports() {
    ...
    const fetchReports = async () => {
        ...
        try {
            ...
            // Assuming the API returns all reports involved with the user (sent by user)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Backend model has receiverRole default 'admin'. So it's fine.
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 bg-white dark:bg-gray-800 p-6 rounded-lg shadow h-fit">
                <h2 className="text-xl font-bold mb-4 dark:text-white">Gửi Báo Cáo / Phản Hồi</h2>
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        <label className="block text-sm font-medium mb-1 dark:text-white">Tiêu đề</label>
                        <input
                            type="text"
                    ...
                    <div className="mb-4">
                        <label className="block text-sm font-medium mb-1 dark:text-white">Loại</label>
                        <select
                            className="w-full border p-2 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            value={type}
                    ...
                    <div className="mb-4">
                        <label className="block text-sm font-medium mb-1 dark:text-white">Nội dung</label>
                        <textarea
                            className="w-full border p-2 rounded h-32 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            value={content}
            ...
            <div className="md:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                <h2 className="text-xl font-bold mb-4 dark:text-white">Lịch Sử Báo Cáo</h2>
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                                            ...
                                            <span className="font-semibold ml-1">{report.sender.fullName}</span>
                                        </p>
                                        <p className="text-gray-800 dark:text-gray-200">{report.content}</p>
                                        <p className="text-xs mt-2 text-gray-500">Trạng thái: <span className="font-bold uppercase">{report.status}</span></p>
                                    </div>
                                    <span className="bg-gray-200 dark:bg-gray-700 dark:text-white text-gray-700 text-xs px-2 py-1 rounded uppercase font-bold">
                                        {report.type}
                                    </span>
                                </div>
                            </div>
